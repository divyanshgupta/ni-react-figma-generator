...
function getWorkaroundTagName(tagName:string){
    if(tagName == "Frame 24517385")
    return "Ni Carousel"
    return tagName;
}
function buildImports(tag: Tag){
   
  const componentMappedToTag = getWorkaroundTagName(tag.name);

  switch (componentMappedToTag) {
    case "Ni Carousel":
      return "import @naukri-ui-dev/ni-carousel";
      break;
...
export function buildCode(tag: Tag, css: CssStyle): string {
  return `${buildImports(tag)}
  const ${capitalizeFirstLetter(tag.name.replace(/\s/g, ''))} = () => {
  return (
